...
function BuildStageProblem(InputParameters::InputParam, SolverParameters::SolverParam, Battery::BatteryParam)       #, state_variables::states When we have 2 hydropower plants- 2 turbines
    ...
    @variable(M, 0 <= auxiliary[iStep=1:NSteps] <= Big, base_name = "Auxiliary")
    
    @variable(M, 0 <= deg[iStage=1:NStages] <= Big, base_name = "Degradation discharge")
    ...
    @constraint(M, degradation[iStage=1:NStages], deg[iStage] >= (sum((n_discharge[iStep]/Eff_discharge + n_charge[iStep]*Eff_charge)*auxiliary[iStep] for iStep=((iStage-1)*NHoursStage+1):(NHoursStage*iStage))*energy_Capacity/max_disc*NHoursStep)/(2*Nfull*energy_Capacity))
  
    @constraint(M,soh[iStage=1:(NStages-1)], soh_new[iStage+1] >= soh_final[iStage])

    @constraint(M,final_soh[iStage=1:NStages], soh_final[iStage] == soh_new[iStage]- deg[iStage])     #deg2
