...
function BuildStageProblem(InputParameters::InputParam, SolverParameters::SolverParam, Battery::BatteryParam)       #, state_variables::states When we have 2 hydropower plants- 2 turbines
  ...
  #4380
   ...
   # NSteps=200
    ...
    @variable(M, 0 <= ch_quad[iStep=1:NSteps] <= (energy_Capacity)^2, base_name = "Auxiliary charge")
    @variable(M, 0 <= dis_quad[iStep=1:NSteps] <= (energy_Capacity)^2, base_name = "Auxiliary discharge")
    @variable(M, z[iStep=1:NSteps] >= 1, base_name = "Auxiliary z")
    ...
    # UPDATE OJECTIVE function - length(Battery_price) = NStages+1=21
    ...
    @constraint(M,equivalent_cycles[iStage=1:NStages], Ncycle[iStage] == sum(ch_quad[iStep]+dis_quad[iStep] for iStep=((iStage-1)*NHoursStage+1):(NHoursStage*iStage))*0.5/(energy_Capacity)^2)  #((energy_Capacity+max_SOH)/2) 
